...
public class EnchereDAOImpl implements EnchereDAO {
	...
	public List<Enchere> getEncheresByEncherisseur(Utilisateur user) {
		...
		try (Connection conn = ConnectionProvider.getConnection()) {
			...
			while (rs.next()) {
				Enchere enchere = new Enchere(rs.getInt(1), rs.getDate(2).toLocalDate(), rs.getInt(3), rs.getInt(4),
	...
	public Enchere getEnchereByNumArticle(int idArticle) {
		...
		try (Connection conn = ConnectionProvider.getConnection()) {
			...
			PreparedStatement stmt = conn.prepareStatement(GET_ENCHERE_BY_ID_ARTICLE,
